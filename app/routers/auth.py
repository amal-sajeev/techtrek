...
def _safe_next(url: str) -> str:
    """Return url only if it is a safe relative path on this site."""
...
def _validate_password(password: str) -> list[str]:
    """Return a list of human-readable errors, empty if the password is strong."""
    errors: list[str] = []
    if len(password) < 8:
...
async def login(request: Request, db: Session = Depends(get_db), _csrf: None = Depends(require_csrf_form)):
    ...
    # Look up by HMAC hash – never by plaintext (column stores ciphertext).
...
async def register(request: Request, db: Session = Depends(get_db), _csrf: None = Depends(require_csrf_form)):
    ...
    # Uniqueness checks via HMAC hash (encrypted fields cannot be compared directly)
...
async def profile_update(request: Request, db: Session = Depends(get_db), _csrf: None = Depends(require_csrf_form)):
    ...
    log_activity(
        db, category="auth", action="profile_update",
...
def speaker_invite_accept(request: Request, token: str, db: Session = Depends(get_db)):
    ...
    if user_id:
        ...
        if user:
            ...
            speaker.invite_token_expires = None
            log_activity(
                db, category="auth", action="invite_accepted",
                description=f"{user.username} accepted speaker invite for '{speaker.name}'",
                request=request, user_id=user.id, target_type="speaker", target_id=speaker.id,
            )
            db.commit()
...
def logout(request: Request, db: Session = Depends(get_db)):
    ...
    if user_id:
        log_activity(
            db, category="auth", action="logout",
            description="User logged out",
            request=request, user_id=user_id, target_type="user", target_id=user_id,
        )
        db.commit()
    request.session.clear()
